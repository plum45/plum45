...
async function handleAgentActions(ctx, type, data, userId, options = {}) {
    ...
    if (IS_RENDER && isLocalAction(type)) {
        try {
            ...
            await ctx.reply(`📡 **ส่งคำสั่ง ${type} ไปยังคอมพิวเตอร์ที่บ้านแล้วค่ะ!**\n\nถ้าคอมเปิดอยู่ จะทำงานทันทีค่ะ\nถ้าคอมปิดอยู่ จะทำงานทันทีที่เจ้านายเปิดเครื่องค่ะ ✨`);
            return;
        } catch (err) {
    ...
    switch (type) {
        ...
        case 'RECOVER_WIFI': {
            // Execution is primarily handled by bridge.js (executeLocalCommand)
            // But if running locally without bridge, we implement it here too.
            ...
            try {
                ...
                let report = `📶 **Wi-Fi Password Recovery**\n\n`;
                const profilesToScan = (data.ssid || data.name) ? profileNames.filter(n => n.toLowerCase().includes((data.ssid || data.name).toLowerCase())) : profileNames.slice(0, 10);
                for (const ssid of profilesToScan) {
                    try {
                        const detail = execSync(`netsh wlan show profile name="${ssid}" key=clear`).toString();
                        const key = detail.match(/Key Content\s*:\s*(.*)/i) || detail.match(/เนื้อหาคีย์\s*:\s*(.*)/i);
                        report += `🔹 **SSID:** \`${ssid}\`\n   🔑 **Pass:** \`${key ? key[1].trim() : '(Open)'}\`\n\n`;
                    } catch (e) { report += `🔸 **SSID:** \`${ssid}\` (Error)\n\n`; }
                }
                await smartReply(ctx, report);
